...
class UserModelTestCase(unitest.TestCase):
    ...
    def test_password_verifications(self):
        u = User(password = 'password')
    ...
    def test_invalid_email_change_token(self):
        ...
        db.session.add(u2)
        db.session.commit()
        token = u1.generate_email_change_token('otherotheruser@example.org')
    ...
    def test_roles_and_oremissions(self):
        Role.insert_roles()
        u = User(email='user@example.com', password = 'password')
        self.assertTrue(u.can(Permission.GENERAL))
        self.assertFalse(u.can(Permission.ADMINSTER))
        
    def text_make_administrator(self):
        Role.insert_roles()
        u = User(email= 'user@example.com', password = 'password')
        self.assertFalse(u.can(Permission.ADMINSTER))
        u.role = Role.query.filter_by(
            permissions=Permission.ADMINSTER).first()
        self.assertTrue(u.can(Permission.ADMINSTER))
        
    def test_administrator(self):
        Role.insert_roles()
        r = Role.query.filter_by(permissions=Permission.ADMINSTER)
